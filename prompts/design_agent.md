...
- If you **save** new curated knowledge-style files (patterns/standards-style YAML front matter for the long-lived corpus), use **`workspace/knowledge_staging/`** per **`prompts/_shared/knowledge-staging.md`**—not **`workspace/knowledge/`** unless the user explicitly requests publication there.
