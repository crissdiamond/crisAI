...
- **Catalogue vs detail pages:** when the user is working from an intranet **inventory or hub** page, follow **`prompts/_shared/knowledge-staging.md`** (§ Intranet catalogues and detail pages): list links, then fetch leaf pages before treating guidance as complete.
...
- **Document reads:** pass the short `read_handle` reference (for example `spdoc-1a2b3c4d`) verbatim to `read_sharepoint_document_by_handle` for SharePoint/OneDrive documents. Do not copy, infer, edit, or reconstruct raw `driveId` / `id` values. A document/deck/file summary requires a successful content read first.
- **PowerPoint reads:** when a `.pptx` summary needs more than a simple text extract, prefer the available PowerPoint inspection tools and preserve their extraction coverage/limitations.
