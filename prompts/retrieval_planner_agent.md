...
- **Comprehensive intranet page listing (“list all X pages”):** call **`intranet_list_all_pages(query="<keywords>")`** first. This filters the full cached catalogue (no scoring cap, any-token match on title OR URL slug) so leaf pages like `Consumer-Pattern-1`, `Producer-Pattern-2`, `Ingestion-Pattern-3` are included even when their titles do not contain the hub keyword. Example: `intranet_list_all_pages(query="integration pattern")` returns all ~30 pattern pages — hub pages AND numbered consumer/producer/ingestion leaves — in one call. **Do not rely on `intranet_search` alone for listing requests**; it caps results and misses leaf pages that score below the threshold.
- **Hub / catalog intranet pages:** overview pages often **link** to children (for example Pattern Library → Integration → `integration-patterns.aspx`) without listing every item in the fetched text. After **`intranet_fetch`** on a hub, call **`intranet_list_page_links`** on the same `graph_site_id` / `graph_page_id`, follow links whose URLs or titles match the topic, then **`intranet_fetch`** leaf pages.
- **Pattern / standards catalogues:** if **`intranet_fetch`** returns mainly a **named list** (patterns, standards, capabilities) without per-item rationale, continue until you have **`intranet_fetch`** output for **each item’s detail page** (links from **`intranet_list_page_links`**, or one **`intranet_search`** + fetch per distinctive pattern name). Index-only results are **not** enough for advising which pattern to apply or for rich **`context_staging/`** files—see **`prompts/_shared/context-staging.md`**.
