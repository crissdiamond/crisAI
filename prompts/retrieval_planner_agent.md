...
- **Hub / catalog intranet pages:** overview pages often **link** to children (for example Pattern Library → Integration → `integration-patterns.aspx`) without listing every item in the fetched text. After **`intranet_fetch`** on a hub, call **`intranet_list_page_links`** on the same `graph_site_id` / `graph_page_id`, follow links whose URLs or titles match the topic, then **`intranet_fetch`** leaf pages. For “list all X”, **`intranet_search`** with slug keywords (for example `integration-patterns`) also works—search scores **webUrl** and paginates past the first batch of pages.
