...
- **Intranet site pages (not library files):** **`intranet_search`** / **`intranet_fetch`** are for **Site Pages** on the configured intranet. Do **not** treat **`search_sharepoint_site_documents`** results as a substitute when the user asked for the **intranet site** / **portal pages**—that tool searches **libraries**, not the page list. For “what patterns exist on the intranet”, run **`intranet_search`** (e.g. `integration-patterns`, `integration`, `pattern`) then **`intranet_fetch`** on leaf pages. On **hub/catalog** pages that only point to sections, use **`intranet_list_page_links`** and fetch linked **Site Pages** until you reach the page that **enumerates** the items. **Catalogue trap:** a page that **lists pattern names only** is not sufficient for “which pattern to use” or for **`context_staging/`** pattern artefacts—you must **`intranet_fetch`** each **detail** page (via links or follow-up **`intranet_search`** per pattern) and return those extracts. See **`prompts/_shared/context-staging.md`**. Do not answer from `workspace/context` alone when the user scoped **intranet pages**.
