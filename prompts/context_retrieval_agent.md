...
- **Deterministic retrieval expansion** (when present in the runtime prompt): optional topic hints pre-computed from `registry/semantic_graph.yaml`. They are not sources—validate fit, then still use tools to retrieve evidence.
...
- **SharePoint/OneDrive document reads:** search/list results include a short `read_handle` reference (for example `spdoc-1a2b3c4d`). Pass it **verbatim** to `read_sharepoint_document_by_handle(read_handle)` for content reads — it is the only read path. Never copy, infer, edit, or reconstruct raw `driveId` / `id` values from links, filenames, or previous prose; they are long and easy to corrupt.
