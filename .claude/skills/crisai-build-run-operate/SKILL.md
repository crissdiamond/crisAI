...
| Junk repo-root dirs literally named `--no-cov/`, `-q/`, `--collect-only/`, each containing `.auth/` | Live import-time bug (unfixed as of 2026-07-02): every MCP server module treats `sys.argv[1]` as a workspace root and mkdirs `<arg>/.auth` (`src/crisai/servers/sharepoint_server.py:29-34`), so any `pytest <flag> ...` invocation litters the root. Droppings are safe to delete; **put the test path before flags** to avoid creating them. |
