#!/usr/bin/env bash
# Launch an on-demand Claude reviewer for a crisAI hcom task.
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage: scripts/hcom_claude_review.sh --role ROLE --thread THREAD [options]

Launch an ephemeral Claude reviewer. The orchestrator owns the lifecycle and
should close the reviewer when the related task no longer needs it.
...
case "$ROLE" in
  runtime_claude)
    ...
    ROLE_FILE="reference/development/roles/runtime_claude.md"
    LABEL="run_claude"
    ;;
  gem_claude)
    AREA="gem"
    TAG="crisai-gem-review"
    ROLE_FILE="reference/development/roles/gem_claude.md"
    LABEL="gem_claude"
    ;;
  web_claude)
    AREA="web"
    TAG="crisai-web-review"
    ROLE_FILE="reference/development/roles/web_claude.md"
    LABEL="web_claude"
    ;;
  *)
    echo "Unsupported Claude review role: $ROLE" >&2
    exit 2
    ;;
esac
...
if [[ -z "$LEASE_MINUTES" ]]; then
  LEASE_MINUTES="$MAX_LEASE_MINUTES"
fi
...
')"
  if [[ -n "$pane_id" && -n "$name" ]]; then
    tmux rename-window -t "$pane_id" "$LABEL(${name##*-})" >/dev/null 2>&1 || true
  fi
else
  output="$(run_or_print "${CMD[@]}" 2>&1)"
  printf '%s\n' "$output" >&2
