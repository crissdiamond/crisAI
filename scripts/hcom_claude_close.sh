#!/usr/bin/env bash
# Close ephemeral Claude reviewers launched for crisAI hcom tasks.
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage: scripts/hcom_claude_close.sh [selector] [--dry-run]

Close ephemeral Claude reviewers by hcom name, role, thread, or expired lease.
