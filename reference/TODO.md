...
| TODO-048 | P1 | todo | Session intent sovereignty and living design memory | Long mixed-intent sessions (CRISAI-ADR-014) must carry prior state as a cumulative asset without corrupting per-turn intent. Today the continuation fold pushes the prior turn's raw text into the request-contract classifier, mis-tagging read/list/summarise asks as publish/write (a source `.pptx` filename read as a publish instruction), and the policy gate then kills completed runs. | Per CRISAI-ADR-014: (1) classify per-turn intent/actions/policy from the new user message only, restricting the continuation fold to reference resolution, and make action vocabulary (e.g. `publication_terms`) intent phrases not bare type tokens — first step, fixes the `publish_artifact` mis-classification, with a regression test reproducing a long-session continuation. (2) Give session sources and decisions provenance + lifecycle status (canonical/superseded/stale; active/superseded), grounded on by relevance rather than replayed. (3) Surface grounded prior state inspectably with pin/retire and a confirm-before-costly-write step. Coordinate with TODO-023 (durable memory tuning). |
