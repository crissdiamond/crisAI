...
| TODO-058 | P1 | todo | Guard the Python↔TypeScript UI contract against drift | `ui/packages/contracts/src/index.ts` is a hand-written mirror of the `src/crisai/schemas/ui_*.schema.json` contracts with no code generation and no CI drift check, and the UI unit tests that might catch a mismatch do not run in CI (TODO-027). For a codebase whose core rule is schema-backed machine-critical exchange, the contract's TypeScript twin is currently unguarded prose-by-another-name. | Either generate the TypeScript contract types from the JSON schemas or add a CI check that fails when `index.ts` drifts from the schemas (field-level comparison, not file hash). Document the update workflow for schema changes. The check runs in CI alongside the UI workspace build and blocks merge on drift. Coordinate with TODO-027, which owns running UI tests in CI. |
...
| TODO-059 | P2 | todo | Remove MCP server import-time workspace-root side effect | Every MCP server module resolves `ROOT` from `sys.argv[1]` at import time and creates `<ROOT>/.auth` (e.g. `src/crisai/servers/sharepoint_server.py`), so importing the modules under pytest turns the first pytest argument into a directory: `--no-cov/`, `-q/`, and `--collect-only/` droppings have recurred at the repo root since May 2026. Module import must be side-effect free. | Server modules perform no filesystem writes at import time; workspace-root resolution and `.auth` creation move to server startup and apply only when the module runs as a server process. Existing droppings are removed and a regression test imports every server module asserting no directory is created. Update the stray-directory note in the diagnostics skill (`.claude/skills/crisai-diagnostics-and-tooling/`). |
...
6. Finish `TODO-040`, then address `TODO-040A` and `TODO-045` according to pilot
   ...
   parity depth, accessibility, and usability rather than another rebuild. Take
   `TODO-058` early in this track: the contract drift guard protects every other
   UI change that follows.
...
| TODO-056 | Lean routing for pure source-inventory asks | `fix(routing): keep source inventories on lightweight path` |
