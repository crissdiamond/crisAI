...
| TODO-048 | P1 | todo | Source-grounding backbone: intent sovereignty, durable source anchors, and workspace evidence materialisation | Long mixed-intent sessions (CRISAI-ADR-014) must carry prior state as a cumulative asset without corrupting per-turn intent, and the find → then compare/author workflow must reuse resolved sources instead of re-searching live, mutable OneDrive. Two reproduced failures: (a) the continuation fold pushed prior raw text into the request-contract classifier, mis-tagging read/list/summarise as publish/write; (b) session `Test001` (2026-06-14) — turn 1 resolved the authoritative v2 deck (`sourcedoc {DD876D07}`) and persisted it, but the follow-up "compare the 2 versions, author a knowledge artefact" failed to bind v2 (resolver needs near-full-title restatement), live-searched OneDrive, matched only the `~$` Office lock file, failed to read it, and the policy gate killed the run before authoring. | Per CRISAI-ADR-014 and CRISAI-ADR-015 (integrated single design — the anchor IS the addressable handle to its materialised evidence): (1) classify per-turn intent/actions/policy from the new user message only, restricting the continuation fold to reference resolution, and make action vocabulary intent phrases not bare type tokens — done for `publish_artifact` (PR #20); keep the regression test. (2) Durable source anchors keyed by stable provider identity (`sourcedoc` GUID / driveItem id / `content_id`), carrying provenance, evidence level, revision token, and lifecycle status (canonical/superseded/stale); resolved on reference (identity/alias/version/ordinal) before any live search; junk objects (`~$` lock files, non-OOXML) never become anchors. (3) Workspace evidence materialisation — on confirmation, cache the raw file + an extracted sidecar (text/tables/notes/image descriptions) in a bounded per-task store keyed by source-id + revision; authoring reads the cached copy, never live OneDrive; revision-invalidated. This is an evidence cache, not a DMS (VISION non-goal), and the concrete form of TODO-003. (4) Surface the bound source set at the retrieval checkpoint with pin/retire and confirm-before-costly-write. Coordinate with TODO-003 (evidence cache), TODO-023 (durable memory tuning), TODO-040 (checkpoint surfaces). |
...
| TODO-045 | P2 | todo | Structured artefact editor enhancements | TODO-025 shipped the editor core (per-type registry, Markdown WYSIWYG + source, front-matter split/preserve, JSON/YAML validation). The remaining structured-artefact niceties from the original scope are not yet built. | Add artefact-profile-aware editing on top of the existing MarkdownEditor: form-like fields for front-matter metadata (instead of a raw YAML block), section navigation for long HLDs/options papers, a Mermaid preview/edit affordance for ```mermaid blocks, and profile-driven artefact validation feedback. Reuse the content-type editor registry; no contract change. |
...
| TODO-040A | P2 | todo | Web stage rail auto-scrolls active stage into view | The focused run view (`f57ce9b`) now auto-follows the active agent in the main panel and marks the live stage in the rail with a pulsing indicator; clicking a stage pins it without focus-jumping. The remaining gap is narrow: for long stage lists the rail does not scroll the active item into the visible viewport. | During an active run, the stage rail scrolls the currently running stage into view for long lists, without disrupting keyboard navigation or a user-pinned selection. Verified across mobile and desktop viewports with reduced-motion support. |
...
| TODO-047 | P2 | todo | Portable knowledge artefact validation for the external knowledge repo CI | The curated knowledge base now lives in its own repository synced via `./start knowledge`. Its CI runs only a lightweight front-matter presence check; the full profile validation (required sections, slug dedup, type aliases) lives in crisAI and is not runnable in the knowledge repo's pipeline. | `crisai validate-artefacts` can validate an arbitrary knowledge tree in the knowledge repo's CI (installable or standalone), or a shipped standalone validator mirrors `registry/workspace_artifact_profiles.yaml`. The knowledge repo's PR gate enforces the full profiles, not just front-matter presence. Documented and test-covered. |
