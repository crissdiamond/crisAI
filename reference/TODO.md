...
- shared UI schemas and v1 runtime events now exist, but React web and Ink Gem
  clients still need to be built against that contract.
...
| TODO-040 | P1 | planned | React web and Ink Gem on shared UI contract | The shared `ui_event_v1` and `/api/v1/runs` contract is the foundation, but the current web UI is still static JavaScript and Gem is still a Textual scaffold. Both should consume the same event stream before more UI behaviour is added. | Add a React TypeScript web app and an Ink TypeScript Gem app that consume `/api/v1/runs`, `/api/v1/runs/{id}`, `/api/v1/runs/{id}/events`, and checkpoint endpoints. Keep Python as the runtime engine. Remove the Textual scaffold after Ink parity. Tests cover event consumption, checkpoint UX, final output, and failure states. |
