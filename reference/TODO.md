...
| TODO-055 | P2 | todo | Source-fit title-phrase extraction precision | `infer_source_fit_constraints` extracts spurious title phrases from instruction words (e.g. `linkable`, `authoritative version. Present`) alongside the genuine quoted phrase. Harmless today because title matching is OR over phrases (it only adds leniency, never false rejection), but it weakens the inventory/content-read source-fit gates — a row matching a junk phrase could pass — and pollutes prompts and policy traces. Surfaced while fixing test003 (PR #29). | `_object_title_phrases` / `_relation_title_phrases` stop at instruction and noise tokens so only genuine title phrases are inferred; the test003 message ("…in the title, list all… Present the files…") yields only the intended phrase; the inventory and content-read gates reject off-title rows without leniency leaks and never falsely reject a legitimate inventory; tests cover the noisy-instruction case. |
...
| TODO-057 | P2 | todo | Peer-mode framing-only planner tool scoping | PR #30 made the pipeline retrieval planner framing-only by removing its source-search servers (so it cannot run a competing retrieval that Context Retrieval then ignores), but `run_peer_pipeline` runs stages via a generic loop and was left unchanged, so the peer-mode planner can still retrieve. | The peer-mode retrieval planner stage runs without source-search servers (`sharepoint_docs`/`documents`/`intranet`), matching the pipeline behaviour; single-agent retrieval keeps full tools; covered by a test. |
