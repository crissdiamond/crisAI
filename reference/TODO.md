...
- a mid-pipeline stage failure in peer mode terminates the run with no partial
  save or graceful degradation to the last completed stage;
- per-task session memory compression is not configurable, which causes
  aggressive context collapse on extended architecture sessions;
...
| TODO-022 | P2 | todo | Peer pipeline partial recovery | If a challenger, refiner, or judge stage times out or fails mid-run, the whole peer pipeline terminates hard with no output saved. Given the cost of peer runs, partial save and graceful degradation to the last completed stage are important. | When a non-retrieval peer stage fails, the pipeline saves the last successfully completed stage output, traces the failure with stage identity, and surfaces a recoverable error to the user rather than discarding all upstream work. Behaviour is test-covered. |
| TODO-023 | P2 | todo | Session memory tuning | The default 2-turn / 3000-char compact memory is aggressive for extended architecture sessions with retrieved source material, causing effective context collapse on long iterative runs. The agentic strategy exists but requires manual env-var opt-in. | Session memory limits are configurable per task type or mode in `registry/session_memory.yaml`. Defaults are reviewed and set to reasonable values for the main architecture use cases. Configuration is documented and test-covered. |
