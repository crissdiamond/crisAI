...
Antigravity is supported when its reusable OAuth token is already present. By
default, Antigravity review uses the persisted model selected in `agy`; for the
Claude path, set that once with `agy`, enter `/model`, choose
`Claude Sonnet 4.6 (Thinking)`, and confirm it appears in the footer.

Use Gemini reviewers through Antigravity with:
...
Those commands set `HCOM_TEAM_REVIEW_MODEL_FAMILY=gemini` and default
`HCOM_TEAM_REVIEW_MODEL=gemini-3-flash-preview`, which is passed to
`hcom agy --model`. Override the model with `HCOM_TEAM_REVIEW_MODEL=<model-id>`
when testing another Gemini model.

The generic review launcher verifies reusable auth before using Antigravity. If
an explicit review model is configured, it also verifies that local hcom supports
`hcom agy --model`; otherwise it probes the persisted agy model before launch.
Failed auth or unsupported model selection is caught before a broken reviewer
pane is created.
...
Antigravity. Add `gemini` after `agy` to use the Gemini review model instead of
the persisted Antigravity default. If Antigravity prompts for OAuth, run `agy`
once manually and retry after it can reopen without asking for authentication.
